    ...
    CRYPTONIA_SETTINGS.chartMorris=function() {
        if($("#morris_area_graph").length) {
            ...
            Morris.Area( {
                ...
                ], resize: true, redraw: true, xkey: 'x', ykeys: ['y', 'z'], labels: ['Y', 'Z'], lineColors: ['#9eeeff', '#f46e0f'], pointFillColors: ['#f46e0f']
            }
        ...
        if($("#morris_bar_graph").length) {
            ...
            Morris.Bar( {
                ...
                ], resize: true, redraw: true, xkey: 'x', ykeys: ['y', 'z', 'a'], labels: ['Y', 'Z', 'A'], barColors: ['#9eeeff', '#3F51B5', '#f46e0f']
            }
            ).on('click', function(i, row) {
                console.log(i, row);
            }
        ...
        if($("#morris_line_graph").length) {
            ...
            Morris.Line( {
                element: 'morris_line_graph', data: day_data, resize: true, xkey: 'period', ykeys: ['licensed', 'sorned'], labels: ['Registered', 'Purchase'], lineColors: ['#9eeeff', '#3F51B5'], pointFillColors: ['#f46e0f']
            }
            );
        }
        if($("#morris_donut_graph").length) {
            ...
            Morris.Donut( {
                ...
                ], resize: true, redraw: true, backgroundColor: '#ffffff', labelColor: '#999999', colors: [ "40-#9eeeff-#2a57d7:70-#2a57d7-#2a57d7:90-#2a57d7", '#f7f7f7']
            }
            );
        }
        if($("#morris_negative_graph").length) {
            ...
            Morris.Line( {
                element: 'morris_negative_graph', data: neg_data, resize: true, redraw: true, xkey: 'period', ykeys: ['a'], labels: ['Series A'], lineColors: ['#9eeeff', '#3F51B5'], units: '%'
            }
            );
        }
        if($("#morris_nogrid_graph").length) {
            ...
            Morris.Line( {
                element: 'morris_nogrid_graph', grid: false, resize: true, redraw: true, data: day_data, xkey: 'period', ykeys: ['licensed', 'sorned'], labels: ['Licensed', 'SORN'], lineColors: ['#9eeeff', '#3F51B5']
            }
            );
        }
        if($("#morris_noncontinuous_graph").length) {
            ...
            Morris.Line( {
                element: 'morris_noncontinuous_graph', data: day_data, resize: true, redraw: true, xkey: 'period', ykeys: ['licensed', 'sorned', 'other'], labels: ['Licensed', 'SORN', 'Other'], lineColors: ['#9eeeff', '#3F51B5'], /* custom label formatting with `xLabelFormat` */
        ...
        if($("#morris_stackedbar_graph").length) {
            ...
            Morris.Bar( {
                ...
                ], resize: true, redraw: true, xkey: 'x', ykeys: ['y', 'z', 'a'], labels: ['Y', 'Z', 'A'], barColors: ['#9eeeff', '#3F51B5', '#f46e0f'], stacked: true
            }
            );
        }
    }
